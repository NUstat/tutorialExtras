...
Thanks to the authors who developed the `learnr` (https://github.com/rstudio/learnr/) and `gradethis` (https://github.com/rstudio/gradethis) packages which these functions are derived from. Many thanks to the author of the `sortable` (https://github.com/rstudio/sortable) package and a solution thread by "stefan's on Jun 01, 2022" (...I cannot seem to find again to properly cite) which helped with the code to develop the word bank question type.
...
To insert a question with one or more blanks into a `learnr` quiz, use `question_dropdown()`.

This question style is equivalent to `question_radio()` but allows for a more condensed display. This choice may be preferred when there are a lot of choices to save space, or if you do not want the user to see all of the possible answer choices after the question is submitted.
