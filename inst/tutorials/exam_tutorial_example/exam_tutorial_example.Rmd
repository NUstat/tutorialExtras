...
lock_server("lock",
            ...
            manual = c("App1-desc"),
            manual_pts = c(1))
reset_server("reset", file_name = "exam_ex", package_name = "ISDSfunctions")
```
...
The submitted answer will ALWAYS be marked incorrect when `isds_setup(exam = TRUE)`. This is so users can go back and change their answer within the exam time frame. Once the exam is locked, the exam will display the "correct" feedback if the answer was correct. If you do not want the correct answer to be displayed, set `correct = Submitted` (ie: whatever you defined `incorrect =` to be).
