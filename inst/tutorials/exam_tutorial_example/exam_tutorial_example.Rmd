...
tutorialExtras_setup(is_exam = TRUE, max_retry = 3, retry_cooldown = 24)
...
The submitted answer will ALWAYS be marked incorrect when `tutorialExtras_setup(exam = TRUE)`. This is so users can go back and change their answer within the exam time frame. Once the exam is locked, the exam will display the "correct" feedback if the answer was correct. If you do not want the correct answer to be displayed, set `correct = Submitted` (ie: whatever you defined `incorrect =` to be), you would also then need to change the css so the buttons are the same color.
